...
namespace Assignment1.TestScenes
{
    public class PositionTest : Scene
    {
        ...
        public override void Initialize()
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                Transform transform = CreateTransform(new Vector2(graphics.PreferredBackBufferWidth / graphics.PreferredBackBufferWidth + graphics.PreferredBackBufferWidth * i, 0), MathHelper.ToRadians(0), 1);
            ...
            for (int i = 0; i < 2; i++)
            {
                Transform transform = CreateTransform(new Vector2(graphics.PreferredBackBufferWidth / graphics.PreferredBackBufferWidth + graphics.PreferredBackBufferWidth * i, graphics.PreferredBackBufferHeight), MathHelper.ToRadians(0), 1);
                SpriteRenderer spriteRenderer = CreateSpriterenderer(transform, "LittleStar", Color.White, 1, SpriteEffects.None);
                Star star1 = createStar(transform, spriteRenderer);
                stars.Add(star1);
            }
        ...
        public override void Draw(SpriteBatch _spriteBatch)
        {
            ...
            _spriteBatch.DrawString(SpriteFont, "press NumPad2 to go to next scene", new Vector2(250, 10), Color.Black, 0, Vector2.Zero, 1.25f, SpriteEffects.None, 1);
